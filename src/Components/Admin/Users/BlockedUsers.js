...
<div className="col-lg-6 col-md-6 col-sm-12 all-user-1">
    ...
    <Link to={`/change_user_status/${user._id}/${user.isActive}`}> <button class="btn btn-info">Unblock</button></Link>
</div>
</div>
    </div>
)
        ...
        .then((data)=>{
          data.map((dbData)=>{
            if(dbData._id===admin_id){
                ...
                fetch('https://apibootwale.herokuapp.com/all_blocked_users',
                {
                    method:'POST',
                    headers:{
                        "Accept":"application/json",
